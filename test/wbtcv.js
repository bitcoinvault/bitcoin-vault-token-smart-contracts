...
/// ALERT FEATURE
  ...
  it('should add recovery address', async () => {
    assert.equal(await instance.recoveryAddresses(accounts[0]), "0x0000000000000000000000000000000000000000");
    await instance.setRecoveryAddress(accounts[2], {from: accounts[0]});
    assert.equal(await instance.recoveryAddresses(accounts[0]), accounts[2]);
  });

  it('should delete recovery address', async () => {
    await instance.setRecoveryAddress(accounts[2], {from: accounts[0]});
    assert.equal(await instance.recoveryAddresses(accounts[0]), accounts[2]);
    await instance.deleteRecoveryAddress({from: accounts[0]});
    assert.equal(await instance.recoveryAddresses(accounts[0]), "0x0000000000000000000000000000000000000000");
  });
  ...
  it('should clean incoming alerts after ready alerts pushed', async () => {
    ...
    incomingAlerts = await instance.pushReadyAlerts(accounts[1]);
    incomingAlerts = await instance.getIncomingAlerts(accounts[1]);
  ...
  it('should not allow blocked user to make alert transfer', async () => {
    await instance.mint(accounts[1], 3);
    await instance.setRecoveryAddress(accounts[2], {from: accounts[1]});
    await instance.blockUser(accounts[1])
    await expectRevert(instance.transfer(accounts[2], 2, {from: accounts[1]}), 'User is blocked');
  });
